...
def listen_query():#监听查询
    ...
    udp_port=getudp_port()#查询广播发送到的端口
    s.bind(('',udp_port))
    ipList=getIPList()
    while True:#持续监听
        # print('正在监听udp广播')
        ...
        if data[0]=='query':
            ...
            if filepaths==[]:#没有相关文件
                if ttl==0:#ttl为0什么都不做 继续监听
                    continue
                else:#否则继续在自己所知的ip内帮忙转发
                    message='query'+' '+keyword+' '+ip+' '+udp_ack_port+str(ttl)
                    for ipaddr in ipList:
                        s.sendto(message.encode('utf-8'),(ipaddr,udp_port))
...
def listen_ack():
    ...
    if not message:
        ...
        for source in sources:
            ...
            button_download = tk.Button(frame, text='下载', command=lambda: download(source, address))
